...
Sio.on("connection", (socket) => {
  ...
  socket.on("positionUpdate", (positionPlayer) => {
    LoggedPlayers.forEach((player) => {
      if (player.name === positionPlayer[2]) {
        player.posX = positionPlayer[0];
        player.posY = positionPlayer[1];
      }
    });
    Sio.emit("UpdatePlayers", LoggedPlayers);
  });
